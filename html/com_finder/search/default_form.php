...
<form id="finder-search" action="<?php echo JRoute::_($this->query->toUri()); ?>" method="get" class="form-inline">
	...
	<fieldset class="word">
		...
		<input type="text" name="q" id="q" value="<?php echo $this->escape($this->query->input); ?>" class="form-control" />
		<?php if ($this->escape($this->query->input) != '' || $this->params->get('allow_empty_query')) : ?>
			<span class="input-group-btn"><button name="Search" type="submit" class="btn btn-default"><?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?></button></span>
		<?php else : ?>
			<span class="input-group-btn"><button name="Search" type="submit" class="btn btn-default disabled"><?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?></button></span>
		<?php endif; ?>
		</div>
		<?php if ($this->params->get('show_advanced', 1)) : ?>
			<p class="small"><a href="#advancedSearch" data-toggle="collapse"><span class="icon-list" aria-hidden="true"></span> <?php echo JText::_('COM_FINDER_ADVANCED_SEARCH_TOGGLE'); ?></a></p>
		<?php endif; ?>
	</fieldset>
